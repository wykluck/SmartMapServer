...
moodycamel::BlockingConcurrentQueue<cvGIS::BlockImageProcessor::BlockImgStruct> ImageFileReader::s_readBlockImgQueue;
...
ImageFileReader::ProcessResult ImageFileReader::readForProcessing(const cv::String& datasetFilePath, const cv::Rect2i& bbox)
{
	...
	if (gdalDecoderPtr->GetXBlockSize() > 1 && gdalDecoderPtr->GetYBlockSize() > 1)
	{
		...
		blockprocessor.startProcessImg(s_readBlockImgQueue, m_processedBlockImgQueue);
		int imgType;
